    ...
    <main class="__content">
        <div class="content__left">
            ...
            <div class="content_bottom">
                ...
                <span class="icon__text">{{ $t('a_new_member') }} <a class="cursor_pointer" @click="redirectTo({ val: 'userLogin' })">{{ $t('login_nav') }}</a></span>
            </div>
        </div>     
        <div class="content__right">
            <div class="header__nav">
                <a class="nav__item " @click="redirectTo({ val: 'userLogin' })">{{ $t('login_nav') }}</a>
                <a @click="redirectTo({ val: 'userSignUp' })" class="nav__item item__active">{{ $t('sign_up_nav') }}</a>
            ...
            <div class="form">
                ...
                <form @click.prevent action="" class="right__down form__down">
                    ...
                    <div class="form__submit">
                        ...
                        <a class="btn__forgot" @click="this.$router.push('/user-forget-pass')">{{ $t('forgot_password') }}</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    methods:{
        ...
        async signUpForm(){
            ...
            if(!this.v$.$error){
                ...
                await Api.authUser.register(payload).then((res) => {
                    // console.log("success fetch", res)
                    if(res?.data?.status){
                        router.push({ path: '/user-verify-email', query: { email: this.email } })
                    }
